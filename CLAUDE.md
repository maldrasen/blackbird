...
the application to be decoupled from the DOM, with only the modules in `/views` needing to be run from within the 
client. The `bin/run-headless.js` file can be used as a shared bootstrap for loading the application in a headless 
mode.
...
`manifest.json` is generated by `bin/compile-manifest.sh` and adds every file within the `application`, `data`, 
`view`, and `test` directories to the manifest.
