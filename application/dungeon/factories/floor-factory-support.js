global.FloorFactorySupport = (function() {
  ...
  // Stairs take up a single tile of their room, returned here in room-local coordinates. Dry tiles are preferred,
  // and of those we take the closest to the room's center point. Ties go to the first tile in reading order, so the
  // choice never depends on a random roll.
  function pickStairsTile(room) {
    ...
    room.getFootprint().forEach((row, y) => {
      row.forEach((cell, x) => {
        if (cell != null) { tiles.push({ x, y }); }
      });
    });

    if (tiles.length === 0) {
      throw new Error(`Room[${room.getIndex()}] has no tiles to put stairs on.`);
    }

    const dry = tiles.filter(tile => room.getFloor(tile.x, tile.y) === 'default');
    const candidates = (dry.length > 0) ? dry : tiles;
