Ability.register('monster-cast-spell', {
  execute: () => {
    ...
    //       goes off. We'll need to store the spell code, target, and power level in the battle state here. Then if
    //       when the monster's turn next happens, their action must be to cast the spell. At that point the target
    //       may have changed, so we need to select the next available target closest to the original target (random
    //       from list of targets with lowest position distance)
