...
Dialog.register(ArchitypeCode.innocent, DialogKeys.proposeTraining_Eager, context => {
  return  `{T:name} looks up at you with wide frightened eyes, "I... okay. I 
    trust you. Just... be gentle with me?"`;
});

Dialog.register(ArchitypeCode.innocent, DialogKeys.proposeTraining_Willing, context => {
  return `{T:name} hesitates, cheeks flushing pink. "I've never really... but 
    if you think it's okay, then... alright."`;
});

Dialog.register(ArchitypeCode.innocent, DialogKeys.proposeTraining_Reluctant, context => {
  return `{T:name} wrings {T:his} hands, avoiding your gaze. "Training? I don't 
    know if I'm ready for something like that..."`;
});

Dialog.register(ArchitypeCode.innocent, DialogKeys.proposeTraining_Unwilling, context => {
