...
Dialog.register(ArchitypeCode.prude, DialogKeys.proposeTraining_Eager, context => {
  return `{T:name} stiffens, then exhales slowly. "...Fine. That is my purpose here I suppose."`;
});

Dialog.register(ArchitypeCode.prude, DialogKeys.proposeTraining_Willing, context => {
  return `{T:name} crosses {T:his} arms and looks away. "I suppose if it must 
    happen, then it must. Just don't expect me to like it."`;
});

Dialog.register(ArchitypeCode.prude, DialogKeys.proposeTraining_Reluctant, context => {
...
Dialog.register(ArchitypeCode.prude, DialogKeys.proposeTraining_Unwilling, context => {
