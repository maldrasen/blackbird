...
This applies to the training mode as well. Need a general way to handle this...
##### A System for Removing Stealth
The sneak attack skill removes stealth correctly when it's used, but I think we need a more general system for detecting and removing stealth when other abilities are used. Characters are hidden until they use an ability that targets an opponent. Stealth healer is totally a thing. Actions that target other characters on the same side should force a reroll of stealth though. Still better than targeting an opponent which will always drop stealth. Also sneak attack should give a damage multiplier to fast weapons. Also need to update the text.
