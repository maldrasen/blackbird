...
# Bugs
The CharacterFactory, given the options `{ gender:'male' }` will sometimes produce a male nymph, which needs to be impossible. 

---
# Current Task

### Personality Archetypes 
After putting together the personality archetype reports I'm seeing way too many 'reserved' and 'serious' people and not nearly enough playful people. I think I'm going to need to rewrite the personality archetypes, in the opposite way this time, starting with the archetype. If all I have are archetypes there may be no need for the calm, violent, passive values as we can just get them from the archetype.

There's a good opportunity here to reduce complexity as well, by giving each archetype an ero style. Outside of dialog, it would make sense that a pervert, a slut, and a bimbo would suck a dick in a very similar way, so we could just build a slutty style for the sex actions, which don't require a lot of talking. Even if we get the dialog from the style, we could still include dialog tags in the template that would refer back up to the archetype.

### Sexual History Factory
We should also add the Sexual History component now as well. The personality and sexual history are pretty tightly coupled. An innocent character must have no sexual history. A prude might have some though. Sluts and bimbos will have a lot of sexual history. Most characters will have some I think, but there should be a fairly high chance that random characters will be virgins no matter their archetype. 

I think the sexual history component needs to have a key for each sex action, for times this action has been done with the player. I would also want to track number of lovers, number of dicks sucked, etc. And then there are the stats like first kiss, first orgasm, etc. 

Still, this sounds easy enough to just do in one big map.
