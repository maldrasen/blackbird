...
### Skill Crits and Fumbles.
I think crits are generally an interesting mechanic to add to a skill system. In my system though, the dice numbers vary dramatically. A low attribute person would roll a 1d6 and a high attribute person might roll 1d60. So I think we need to roll crits before we actually do a skill roll. Crit chance would have to be managed as a separate attribute, and there could be a global crit chance, and skill specific crit chances. Fumble chances too of course. If we make this change then a skill check wouldn't return a simple number, but an object with details. We need the object to tell us if the result is a crit, and whatever asked for the skill check will need to handle crits on their own. 
...
### Time and Stamina
Every SexAction will also need a time and stamina value. The time it takes to perform the action (usually about a minute) and the stamina cost for doing the action, both for the player and the partner.

### Sexual Skills
In the training action, the sexual skills are really only adding to the baseline sensations. In this case the technique skill is only ever a positive bonus. If we wanted to change this to a skill based check, actions would need a difficulty target we'd have to hit. This makes some sense as certain sex actions, like eating pussy or blowjobs to require some amount of skill. So... I guess we need to add a technique target. If the technique roll falls below the target the sensations are penalized, and anger is added. If we exceed the target by a high amount perhaps there could be bonus sensation, and anger reduction. This way we can also incorporate crits and fumbles to the skill rolls, which is another task entirelly.
