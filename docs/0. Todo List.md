...
### Minimum Consent
We need to add a minimum consent property to sex actions that are impossible to force a person to do. This applies to actions like `lap-dance` and `strip-tease` that require active participation from the partner character.
...
Persisted actions will need to have their consent recalculated every turn. If they fall below the minimum consent, or are successfully fought back against, we need to remove them.

##### Multiple Actions Using the Same Slot...
