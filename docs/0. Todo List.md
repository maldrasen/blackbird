...
---
# Bugs
- If an action is persisted it shouldn't be in the action list.
- Fuck anus is in the action list twice (probably because of the insert anus action)
...
### Sex Positions
Sex positions work with the persisted actions, where if you pick an action that necessitates a position change, the persisted actions that don't work in the new position will need to be removed. For instance, if you're fucking in the doggy style position, and choose blowjob as your next action, that stops the fucking persisted action, repositions the characters to that the blowjob can happen, and persists the blowjob state. In this case we would go from doggie to kneeling and set the character's roles in the new position accordingly. 

The positions will require a graph where we map what positiona change into which. The nodes would be the positions, and the edges the moves needed to reach those positions. Doggy to missionary would just be "partner turns over". Doggy to reverse cowgirl could be a "pulled on top of" move. Each position has a list of positions that it can change to in a single move. There no good way to get from doggy to kneeling so that would require a complete rearrangement. When we rearrange we remove all persisted actions.

We could also add a general 'reposition' action that shows only the positions reachable from the current position, showing what persisted actions would be removed, which we know by looking at the difference between the alignment maps between the two positions.
