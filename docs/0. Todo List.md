...
Clicking on a monster in the formation should give a limited character view that just shows what can be seen and what the character knows about. Things like cock size will be unknown if they're wearing pants for instance. 
##### Battle Effects
We need to show some visible effects when a character is hit. Something like the fading flash square. Color could indicate what kind of hit (normal / crit) and damage type, in other words, each damage type needs a color.
##### Show all battle messages at once
It's annoying to click though all the messages. Better to have one message with multiple lines per turn. 
...
### TrainingSystem vs TrainingController
It's confusing, but it actually makes kind of sense for these to be separate. The training system changes the game mode and can reach into the training views. The training controller is entirely independent of the views in order to make testing easier. I'm thinking these types of modules need better names, and the battle controller and system and everything else needs to follow the same established pattern. Again, this is mostly a naming of things problem, but it's good to have clear boundaries so that components can be isolated and tested.
...
# Battle Tasks
Still laying the initial groundwork for the battles
