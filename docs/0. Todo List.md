...
### Training UI
We now need to show the relevant character details in the training UI. The more important details are the health, stamina, and pleasure. Perhaps a few details about the number of times they've orgasmed, about of cum inside of them. The current training scales will go here as well, though we don't need to worry about displaying anima and animus until the end of training. 

We should go ahead and create the bar element as well, and maybe go back to the character overlay and integrate the bar elements. We'll need two versions, one that rolls over for the scales and the feelings, and one that has a set range for all the other bars. I think the bars should be fairly thin, with a label on top. Maybe display the values along with the bar.
