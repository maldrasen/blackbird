...
We can randomly assign a kobold (named Scarlet) red hair. Perhaps the hair color mutation should change scale color when the species has scales, but no hair. Some furries, like equins and lupins have both hair and fur though. Some might not though, like the selkies. So we should add a hasHair property to species. Hmm, actually we only have a hair color that we're also using for fur, so maybe add a fur color property to the body as well, that way I don't need to look at the species to determine if there is hair and fur, or one or the other.
