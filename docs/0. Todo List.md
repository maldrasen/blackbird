...
### Personality and Sexual History Factory
Rather than calculating the archetype each time, I think we need to assign one in the personality factory. We should also add the Sexual History component now as well. The personality and sexual history are pretty tightly coupled. An innocent character must have no sexual history. A prude might have some though. Sluts and bimbos will have a lot of sexual history. Most characters will have some I think, but there should be a fairly high chance that random characters will be virgins. 

Part of this task will be turning Archetype into a record class so that they can define a parent archetype to fallback to.

### Extremely Simple Equipment
When writing some of these sex actions I need to know if a character is naked or not. I think I should just add a very simple equipment component with a naked or not flag. The Character wrapper should have `isNaked()`, `isTopless()`, and `isBottomless()` functions. Actions like striptease will need a way to remove clothing, and characters should get dressed again after sex.

There are a couple ways I can see doing equipment. The most straightforward way is for the equipment component to be a list of slots. (shirt, pants, etc.) Equipping an item would put its entity id into the equipped slots. Items would have the slot they occupy. This could get complex because of body piercings, which might have to be an entirely different component. Characters would need an inventory as well, which could either be an array of item entity ids, or a single item registry where each item knows who's carrying it.

### Personality Reports
I want to check the distribution of archetypes across different species. I should make a report for each species that creates 100 characters, then count how many of each archetype we get, and perhaps tweak the species data if we need different archetypes or more variety.

### Training Systems
I now need to get the training system in a workable state. That's the primary focus of the game, so I want to make that work before including anything else like day to day exploration, events, combat, anything that's not directly related to training, stubbing things out as needed. I've got the consent and sensations calculations in a decent state. Now I need to add them to the training scales and start actually chaining actions together.
##### Next Steps
- Implement Training Scales in Sensation Result
...
- Need to look into some of the action requirements. The titfuck actions require a minimum breast size or compatible shape. Striptease requires the partner to be wearing clothes, and is made naked afterwards. (Implement simple equipment before doing that though)
- Need to implement the enlighten view.
...
### Aspect Validations (Trait Monitoring?)
The personality aspects (bimbo, slut, prude, etc.) need to include some validation and monitoring to ensure that associated personality variables are compatible. The bimbo aspect in particular is often assigned via a name trigger that gives the bimbo character a bunch of sexual preferences and lowers their intelligence. We need to add some kind of monitoring and control system that prevents bimbo characters from raising their intelligence when they have this aspect. Bimbos should probably have a minimum calmness set as well, so that it makes sense for their dialog to be bubbly. Bimbo and slut should also be female/futa only aspects to make their dialog trees simpler. It's much easier to assume that a bimbo always has tits. We can add a separate 'himbo' aspects for male bimbos.

# Primary Tasks

### Refactor SensationResult
I was trying to do the sensations all in one place, but the SensationResult needs to be broken into multiple files, probably one for each apply type. These subclasses would take the Sensation Result as an argument and modify its state.
