...
# Current Task
### Battles
Working on all the position text was getting super boring, so I'm pivoting to working on the battle system for a bit. As it is the most important system, I really want to have the training system in a mostly working state, but I think it's also important to have a framework of all the major game systems in place as well. 
##### Implement Vulnerable
We have vulnerable applied when the defense roll fumbles, but the attack roll misses. This only rarely happens though, so it's difficult to test in the battle system. We should create an ability that applies vulnerable to a character to make this easier to test. We could also add a special weapon to the game that inflicts the vulnerable state, that sounds like an interesting task.
##### Implement Stealth
One of the first abilities I'd like to develop is stealth. A character can only hide when they're in the back rank. When they are hidden they can't be targeted. They are hidden until they use an ability that targets an opponent. Stealth healer is totally a thing. Actions that target other characters on the same side should force a reroll of stealth though. Still better than targeting an opponent which will always drop stealth. When hidden the basic attack is hidden and the sneak attack is shown instead. Sneak attack can hit any position, has an attack accuracy bonus, and extra damage, but can only hit a single time. Interesting meta here, a sneak attack with a maul would be much more effective than a sneak attack with a dagger. Perhaps sneak attack should give a damage multiplier to fast weapons as well to prevent this...
##### Implement Dick Punch
The dick punch skill can only target a dick haver. If a person's cock is armored (metal legs equipped) its only as effective as a regular punch to the legs. If not this attack stuns and deals damage scaling with cock size and sensitivity.
##### Using skills should improve them
This applies to the training mode as well.
##### Character Speed and reaction time
We're just randomly picking a number for character's reaction time, but we need to determine a speed factor. Speed probably comes from a lot of different factors, like dexterity and even height. (Taller characters are slower) Could even take things like breast size into consideration, as having a pair of huge swinging milkers will slow a person down significantly. Speed also effects number of attacks and could reduce delay between actions.
##### Text changes for Proper Weapon Names
Kind of a nitpick but when a weapon has a generic name like "dagger" the attack text "swings his dagger at someone" makes sense, but when you give a weapon a proper name, like "Stabitha", the attack text should be "swings Stabitha at someone". The loom will need to determine if a weapon has a proper name, or a generic name. The names given by applying an enchantment are probably still generic (He thrusts his dagger of facestabbing) but unique weapons will definitely have proper names.

### Dungeon Stuff
Like the battles, I think I want to do some foundation work on the dungeons before going back to work on the training system. As before, the current goal here is to populate a dungeon level with features up to a target density. Add smaller corridors to make sure that everything is reachable. Then add doors to make sure that every part of the dungeon can be reached.
