The game will have no way to save a dungeon floor. Technically we might be able to save between floors, but that would only save the current floor number. The saved game state would need to persist the game mode and the floor number on save then.
