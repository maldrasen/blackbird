...
### Notes
- **Appraisal moved to init.** Building abilities at registration meant the venomous bite would have priced its poison before the poison status type existed, so `AbilityAppraiser.run()` runs after the compile pass. It's still a stub, and the essence system's per-entry breakdown and the essence report script still read the old map. The two pending specs in `essence-system-spec.js` carry the values the appraiser has to reproduce.
