...
We're going to have to update the orchard so that the trees are more spread out so that the party can actually walk through the orchard. Currently, we're drawing about 2 trees a tile. Instead we need to update the orchard so that there's a row of trees, then an empty row, and every other tile in the tree row needs to be empty. The outer parameter also needs to remain empty so that the trees can't block the doors. We may need to eliminate the smaller sized orchards. A 7x7 room, containing three rows of three trees would probably be the minimum.
