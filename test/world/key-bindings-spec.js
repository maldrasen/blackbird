describe("KeyBindings", function() {
  ...
  it("binds a default key to every battle command", function() {
    ...
    Object.values(BattleCommandCode).forEach(code => {
  ...
  it("finds the action bound to a key in a context", function() {
    expect(KeyBindings.getAction('battle','KeyA')).to.equal(BattleCommandCode.basicAttack);
  ...
  it("reads saved bindings over the defaults", async function() {
    await WorldState.setOptions({ keyBindings:{ battle:{ [BattleCommandCode.basicAttack]:'KeyQ' } } });

    expect(KeyBindings.getBinding('battle',BattleCommandCode.basicAttack)).to.equal('KeyQ');
    expect(KeyBindings.getAction('battle','KeyQ')).to.equal(BattleCommandCode.basicAttack);
    expect(KeyBindings.getAction('battle','KeyA')).to.equal(null);
    expect(KeyBindings.getAction('battle','KeyD')).to.equal(BattleCommandCode.basicDefend);
    expect(KeyBindings.getBinding('dungeon','north')).to.equal('KeyW');
  });

  it("lets an action be unbound", async function() {
    await WorldState.setOptions({ keyBindings:{ battle:{ [BattleCommandCode.basicAttack]:null } } });

    expect(KeyBindings.getBinding('battle',BattleCommandCode.basicAttack)).to.equal(null);
    expect(KeyBindings.getAction('battle','KeyA')).to.equal(null);
  ...
  it("hands out a fresh copy of the bindings each time", function() {
    KeyBindings.getBindings().battle[BattleCommandCode.basicAttack] = 'KeyZ';
    expect(KeyBindings.getBinding('battle',BattleCommandCode.basicAttack)).to.equal('KeyA');
  });
  ...
  describe("findConflicts()", function() {
    it("flags a key bound to two actions in one context", function() {
      ...
      bindings.battle[BattleCommandCode.basicDefend] = 'KeyA';
      ...
      expect(conflicts[0].actions).to.have.members([BattleCommandCode.basicAttack, BattleCommandCode.basicDefend]);
    });

    it("allows the same key in different contexts", function() {
      const bindings = KeyBindings.getDefaults();
      bindings.battle[BattleCommandCode.basicAttack] = 'KeyW';
      expect(KeyBindings.findConflicts(bindings)).to.deep.equal([]);
    });

    it("ignores unbound actions", function() {
      const bindings = KeyBindings.getDefaults();
      bindings.battle[BattleCommandCode.basicAttack] = null;
      bindings.battle[BattleCommandCode.basicDefend] = null;
      expect(KeyBindings.findConflicts(bindings)).to.deep.equal([]);
    });
  });
